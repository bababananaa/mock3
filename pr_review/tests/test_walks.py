...
def test_payout_statement(client):
    response = client.get("/walkers/1/payouts?from=2026-09-01&to=2026-09-15&page=0&page_size=10")
    assert response.status_code == 200
    body = response.get_json()
    assert len(body["walks"]) == body["total_walks"]
    assert body["total_earnings_cents"] > 0


def test_payout_statement_requires_dates(client):
    assert client.get("/walkers/1/payouts").status_code == 400
